...
export function TaxModuleLayout({ moduleTitle, basePath, currentStep, summary, children, showModuleHeader = true }) {
  const stepIndex = STEP_ORDER.indexOf(currentStep);
  const progress = ((stepIndex + 1) / STEP_ORDER.length) * 100;
  return <Container maxWidth={false} sx={{ py: 1.5, px: { xs: 1.5, md: 2.5 }, width: '100%' }}><Stack spacing={1.25}>{showModuleHeader && <>
      <Typography variant='h5' sx={{ fontWeight: 800 }}>{moduleTitle}</Typography>
