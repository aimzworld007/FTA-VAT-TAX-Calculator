...
function HomeContent() { return <><Box sx={{ bgcolor: '#fff', border: `1px solid ${SHELL.colorBorder}`, borderRadius: SHELL.radiusCard, p: { xs: 2, md: 3 }, minHeight: { md: 205 } }}><Stack direction={{ xs: 'column', md: 'row' }} justifyContent='space-between' gap={2} sx={{ height: '100%' }}><Box><Typography sx={{ color: '#64748b', fontWeight: 600, fontSize: '2rem' }}>Welcome to</Typography><Typography sx={{ color: '#0f1f49', fontWeight: 800, fontSize: { xs: '2rem', md: '3rem' }, lineHeight: 1.2 }}>UAE VAT & Tax Assistant</Typography><Typography sx={{ mt: 1.2, color: '#55627c', fontSize: '1.15rem' }}>Your all-in-one workspace for FTA compliance and tax management</Typography></Box></Stack></Box>
  <Typography sx={{ fontWeight: 800, fontSize: '1.95rem', color: '#0f172a', px: 0.4 }}>Choose a tax module</Typography>
  <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 1fr' }, gap: 2 }}>{moduleItems.map((item) => <Box key={item.title} component={RouteLink} to={item.to} sx={{ textDecoration: 'none', color: 'inherit', bgcolor: '#fff', border: `1px solid ${SHELL.colorBorder}`, borderRadius: SHELL.radiusCard, p: 2.3, boxShadow: '0 2px 8px rgba(15,23,42,0.04)' }}><Stack direction='row' gap={1.6} alignItems='center'><Box sx={{ width: 64, height: 64, border: `1px solid ${SHELL.colorBorder}`, borderRadius: 1.2, p: 0.6, display: 'grid', placeItems: 'center' }}><Box component='img' src={item.image} alt={item.title} sx={{ width: '100%', height: '100%', objectFit: 'contain' }} /></Box><Box sx={{ flex: 1 }}><Typography sx={{ fontWeight: 800, fontSize: '1.7rem' }}>{item.title}</Typography><Typography sx={{ color: '#58657e', mt: 0.4, fontSize: '1.02rem' }}>{item.description}</Typography></Box><Box sx={{ width: 44, height: 44, borderRadius: SHELL.radiusButton, bgcolor: SHELL.colorPrimary, color: '#fff', display: 'grid', placeItems: 'center', flexShrink: 0 }}><ArrowForwardRoundedIcon fontSize='small' /></Box></Stack><Stack direction='row' flexWrap='wrap' gap={1} sx={{ mt: 1.8, pt: 1.6, borderTop: `1px solid ${SHELL.colorBorder}` }}>{['Guided workflow', 'Live summaries', 'Export options'].map((chip) => <Chip key={chip} label={chip} sx={{ height: 32, borderRadius: 10, bgcolor: '#f1f5fb', '& .MuiChip-label': { px: 1.3, fontSize: '0.82rem' } }} />)}</Stack></Box>)}</Box></>; }
