...
import { BookOpenText, Building2, FileSpreadsheet, ShieldCheck } from 'lucide-react';
...
function HomeContent() { return <><Box sx={{ background: 'linear-gradient(135deg, #eff6ff 0%, #ffffff 55%, #dbeafe 100%)', border: `1px solid ${SHELL.colorBorder}`, borderRadius: SHELL.radiusCard, p: { xs: 2, md: 3.2 }, minHeight: { md: 220 }, boxShadow: '0 8px 30px rgba(37,99,235,0.08)' }}><Stack direction={{ xs: 'column', md: 'row' }} justifyContent='space-between' gap={2.2} sx={{ height: '100%' }}><Box sx={{ maxWidth: 860 }}><Typography sx={{ color: '#64748b', fontWeight: 700, fontSize: { xs: '1rem', md: '1.08rem' }, letterSpacing: '0.02em' }}>Welcome to</Typography><Typography sx={{ color: '#0f1f49', fontWeight: 800, fontSize: { xs: '1.9rem', md: '2.7rem' }, lineHeight: 1.2 }}>UAE VAT & Tax Assistant</Typography><Typography sx={{ mt: 1.2, color: '#475569', fontSize: { xs: '0.98rem', md: '1.1rem' } }}>Your all-in-one workspace for FTA compliance and tax management</Typography><Stack direction='row' gap={1} flexWrap='wrap' sx={{ mt: 1.8 }}>{['FTA Ready', 'Secure', 'Smart Workflow'].map((chip) => <Chip key={chip} label={chip} sx={{ height: 30, borderRadius: 999, bgcolor: '#fff', border: '1px solid #bfdbfe', '& .MuiChip-label': { fontWeight: 600, color: '#1d4ed8' } }} />)}</Stack></Box><Box sx={{ alignSelf: 'center', display: { xs: 'none', md: 'block' } }}><Box sx={{ width: 180, height: 160, borderRadius: 3, border: '1px solid #bfdbfe', background: 'linear-gradient(145deg,#ffffff 0%, #dbeafe 100%)', p: 1.2, boxShadow: '0 12px 28px rgba(37,99,235,.14)', display: 'grid', gap: 1 }}><Box sx={{ height: 42, borderRadius: 2, bgcolor: '#eff6ff', display: 'flex', alignItems: 'center', px: 1.2, color: '#1d4ed8', gap: 1 }}><ShieldCheck size={18} /><Typography sx={{ fontWeight: 700, fontSize: '.78rem' }}>FTA Compliance</Typography></Box><Box sx={{ height: 42, borderRadius: 2, bgcolor: '#fff', border: '1px solid #dbeafe', display: 'flex', alignItems: 'center', px: 1.2, color: '#1e3a8a', gap: 1 }}><BookOpenText size={18} /><Typography sx={{ fontWeight: 700, fontSize: '.78rem' }}>Guided Filing</Typography></Box><Box sx={{ flex: 1, borderRadius: 2, background: 'linear-gradient(145deg,#2563eb 0%, #60a5fa 100%)' }} /></Box></Box></Stack></Box>
