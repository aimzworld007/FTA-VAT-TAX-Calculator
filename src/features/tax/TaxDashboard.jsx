...
const vatDefault = { businessName: '', trn: '', taxPeriodStart: '', taxPeriodEnd: '', filingFrequency: 'Quarterly', standardRatedSales: 0, zeroRatedSales: 0, exemptSales: 0, standardRatedPurchases: 0, recoverableInputVat: 0, nonRecoverableVat: 0, previousAdjustment: 0, badDebtRelief: 0, adjustmentNotes: '', monthlyEntries: [] };
...
export function TaxDashboard() {
  ...
  const [vat, setVat] = React.useState(() => {
    const loaded = draftStorage.load('vatDraft', vatDefault);
    return { ...loaded, monthlyEntries: buildMonthlyEntries(loaded) };
  });
  ...
  return <main className='content'><header className='hero'><div><p className='eyebrow'>UAE Tax Calculator</p><h1>UAE VAT & Corporate Tax Calculator</h1><p>Prepare VAT returns and corporate tax estimates with guided calculations.</p></div><div className='hero-card'><span className='badge'>{TAX_CONFIG.badge}</span></div></header>
    ...
    {mode === 'vat' && <VatWizard data={vat} setData={setVat} onSave={() => draftStorage.save('vatDraft', vat)} onReset={() => { if (confirm('Reset VAT draft?')) { const next = { ...vatDefault, monthlyEntries: buildMonthlyEntries(vatDefault) }; setVat(next); draftStorage.clear('vatDraft'); } }} />}
