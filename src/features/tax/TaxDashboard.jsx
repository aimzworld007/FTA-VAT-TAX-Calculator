...
import { calculateVat } from './lib/vatCalculator';
...
const vatDefault={businessName:'',trn:'',taxPeriodStart:'',taxPeriodEnd:'',filingFrequency:'Quarterly',standardRatedSales:0,zeroRatedSales:0,exemptSales:0,standardRatedPurchases:0,recoverableInputVat:0,nonRecoverableVat:0,previousAdjustment:0,badDebtRelief:0,adjustmentNotes:''};
...
export function TaxDashboard(){const [mode,setMode]=React.useState(''); const [vat,setVat]=React.useState(()=>draftStorage.load('vatDraft',vatDefault)); const [ct,setCt]=React.useState(()=>draftStorage.load('ctDraft',ctDefault)); const v=calculateVat(vat); const c=calculateCorporateTax(ct);
...
<section className='card'><h2>Live Summary</h2><div className='grid-section'><TaxSummaryCard label='VAT Payable / Refundable' value={money(v.netVat)}/><TaxSummaryCard label='Corporate Tax Estimate' value={money(c.taxPayable)}/><TaxSummaryCard label='Selected Tax Period' value={vat.taxPeriodStart&&vat.taxPeriodEnd?`${vat.taxPeriodStart} → ${vat.taxPeriodEnd}`:'Not selected'}/><TaxSummaryCard label='Completion Progress' value={`${completion}%`}/></div><div className='selector-grid no-print'><TaxModeCard title='VAT Return' desc='Open VAT return wizard' onClick={()=>setMode('vat')} active={mode==='vat'}/><TaxModeCard title='Corporate Tax' desc='Open corporate tax wizard' onClick={()=>setMode('ct')} active={mode==='ct'}/></div></section>
{mode==='vat'&&<VatWizard data={vat} setData={setVat} onSave={()=>draftStorage.save('vatDraft',vat)} onReset={()=>{if(confirm('Reset VAT draft?')){setVat(vatDefault);draftStorage.clear('vatDraft');}}}/>} 
