...
function Header({ onOpenDrawer, onGoHome }) {
  ...
  return <AppBar position='sticky' color='inherit' elevation={0} sx={{ borderBottom: '1px solid #dbe3ef', backgroundColor: 'rgba(255,255,255,0.93)', backdropFilter: 'blur(8px)' }}><Toolbar sx={{ justifyContent: isDesktop ? 'space-between' : 'center', gap: 1.5, py: 1, position: 'relative' }}><Stack direction='row' spacing={1.25} alignItems='center'><IconButton sx={{ display: { lg: 'none' }, position: 'absolute', left: 8 }} onClick={onOpenDrawer}><MenuIcon /></IconButton><IconButton aria-label='Go to homepage' onClick={onGoHome} sx={{ p: 0.25 }}><Box component='img' src='/logo.png' alt='logo' sx={{ width: { xs: 108, sm: 120 }, height: { xs: 40, sm: 44 }, objectFit: 'contain', borderRadius: 1 }} /></IconButton></Stack>
...
export function TaxDashboard() {
  ...
  return <Box sx={{ pb: { xs: 8, md: 0 } }}><Header onOpenDrawer={() => setDrawerOpen(true)} onGoHome={() => setM('home')} />
