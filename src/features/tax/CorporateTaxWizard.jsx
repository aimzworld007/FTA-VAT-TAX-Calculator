...
const F=({label,v,on,n='text'})=><Grid size={{xs:12,md:4}}><TextField fullWidth type={n} label={label} value={v} onChange={on} /></Grid>;
...
</FormSection><Stack direction={{xs:'column',sm:'row'}} spacing={1.5} sx={{mt:2}}><Button variant='outlined' onClick={()=>setStep(s=>Math.max(1,s-1))} disabled={step===1}>Back</Button><Button className='primary-gradient-btn' onClick={()=>setStep(s=>Math.min(6,s+1))} disabled={step===6 || (step===1 && missingRequired)}>Continue</Button></Stack>{step===6&&<ExportActions onSave={onSave} onReset={onReset} onPrint={()=>window.print()} onPdf={()=>downloadPdfReport({ reportId: 'corporate-tax-report', reportType: 'corporate-tax', companyName: data.companyName, taxPeriod: data.financialYearStart && data.financialYearEnd ? `${data.financialYearStart}_to_${data.financialYearEnd}` : 'period' })}/>}</div>}
