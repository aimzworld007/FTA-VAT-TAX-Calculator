        ...
        <CardContent sx={{ py: 0.9, px: 1.1, '&:last-child': { pb: 0.9 } }}>
          <Stack direction='row' spacing={1} alignItems='center'>
            ...
            <Box sx={{ minWidth: 0, flex: 1 }}>
              ...
              <LinearProgress variant='determinate' value={progress} sx={{ mt: 0.6, height: 6, borderRadius: 999, backgroundColor: '#e8eef7' }} />
            </Box>
          </Stack>
        </CardContent>
      </Card>
...
export function ExportActions({onSave,onReset,onPrint,onPdf,pdfLoading}){return <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} className='wizard-action-group wizard-action-group-right'><Button variant='outlined' onClick={onSave}>Save Draft</Button><Button color='warning' variant='outlined' onClick={onReset}>Reset</Button><Button variant='outlined' onClick={onPrint}>Print</Button><Button variant='contained' onClick={onPdf} disabled={pdfLoading}>{pdfLoading?'Generating PDF…':'Download PDF'}</Button></Stack>;}
